...
function getBuiltinDialogResponse(text) {
  ...
  // 캐릭터 교체
  if ((lower.includes("접수원") || lower.includes("우주")) && hasKeyword(lower, ["불러", "바꿔", "교체", "호출", "로 해", "로 바꿔"])) {
    if (typeof setCurrentCharacter === "function") setCurrentCharacter("greeter");
      ...
      if (typeof currentCharacterName !== "undefined") currentCharacterName = "우주";
    }
    const ghostEl = document.getElementById("ghost");
    if (ghostEl) ghostEl.classList.add("char-greeter");
    clearDialogState();
    return { emotion: "기쁨", line: buildCharacterSwitchLine(getActiveCharacterName("우주"), "switch") };
  }
