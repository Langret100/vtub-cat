...
function setModeSocial(enabled) {
  ...
  function patchHandleUserSubmit() {
    ...
    window.handleUserSubmit = async function () {
      ...
      // 캐릭터 이름 호출 감지: GhostCoreBridge 사용
      var bridge = window.GhostCoreBridge || null;
      var charName = bridge ? bridge.getCurrentCharacterName() : "";
      var charQuestion = null;
      if (charName && bridge) {
        try { charQuestion = bridge.extractCharacterCallText(text); } catch(e) {}
      }

      if (charQuestion !== null && charQuestion !== undefined) {
        // 1) 사용자 메시지 전송 (sendSocialMessage 내부에서 input 비움)
        sendSocialMessage(text);
        // 2) AI 응답 생성 → 캐릭터 이름으로 채팅창 전송
        try {
          var question = charQuestion || text;
          var aiReply = await bridge.getUnifiedCharacterChatResponse(question, { allowCharacterCall: false });
          if (aiReply && typeof aiReply === "string" && aiReply.trim()) {
            ...
            // 부모 창 캐릭터 말풍선에도 표시
            try {
              if (typeof window.setEmotion === "function") window.setEmotion("기쁨", aiReply.trim());
            } catch(e) {}
          }
        } catch(e) {
          console.warn("[social-chat] 캐릭터 AI 응답 오류:", e);
        }
        return;
      }
